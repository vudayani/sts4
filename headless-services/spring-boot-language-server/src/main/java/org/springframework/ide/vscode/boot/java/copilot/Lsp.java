...
public class Lsp {
	...
	public record TextDocumentEdit(TextDocumentIdentifier textDocument,
			List<TextEdit> edits) implements ChangeOperation {
	}

	public record CreateFileOptions(boolean overwrite, boolean ignoreIfExists) {
	}

	public record CreateFile(String kind, String uri, CreateFileOptions options,
			String annotationId) implements ChangeOperation {
	...
	public record DeleteFile(String kind, String uri, DeleteFileOptions options,
			String annotationId) implements ChangeOperation {
	...
	public record WorkspaceEdit(List<ChangeOperation> documentChanges,
